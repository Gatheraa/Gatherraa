...
use crate::{MultisigWalletContract, MultisigWalletContractClient, TransactionStatus};
use soroban_sdk::{
...
impl MaliciousReentrancyMultisigContract {
    pub fn init(env: Env, target: Address, transaction_id: Symbol) {
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "should_reenter"), &true);
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "call_count"), &0u32);
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "target"), &target);
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "transaction_id"), &transaction_id);
    }
    ...
    pub fn malicious_callback(env: Env) -> i128 {
        ...
        env.storage()
            .instance()
            .set(&Symbol::new(&env, "call_count"), &(call_count + 1));
        ...
        if should_reenter && call_count == 0 {
            ...
            env.storage()
                .instance()
                .set(&Symbol::new(&env, "should_reenter"), &false);
...
fn create_test_env() -> (
    ...
    Vec<Address>,
    Address,
    MultisigWalletContractClient<'static>,
) {
    let env = Env::default();
...
fn initialize_multisig_wallet(
    _env: &Env,
    client: &MultisigWalletContractClient,
    owners: &Vec<Address>,
    threshold: u32,
) {
